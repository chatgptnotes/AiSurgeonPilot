...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get doctor details - only if created by this admin clinical
    ...
    if (doctorError || !doctor) {
      return NextResponse.json(
        { error: 'Doctor not found or you do not have permission' },
        { status: 404 }
      )
    }
...
async function sendCredentialsWhatsApp({
  ...
  const message = `Dr. ${fullName}, your AiSurgeonPilot password has been reset.
