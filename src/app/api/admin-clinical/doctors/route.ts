...
import { verifyAdminClinical, isAuthError } from '@/lib/auth/verify-role'
...
// POST - Create new doctor (by admin clinical)
export async function POST(request: NextRequest) {
  try {
    const authResult = await verifyAdminClinical()
    ...
    const { data: newDoctor, error: profileError } = await supabaseAdmin
      .from('doc_doctors')
      .insert({
        ...
        online_fee: onlineFee || null,
        booking_slug: bookingSlug,
        is_verified: verifyImmediately === true,
        must_change_password: true,
        created_by: authResult.doctorId, // Admin Clinical's ID
        role: 'doctor',
        is_active: true,
      })
      .select()
...
// GET - List doctors created by this admin clinical
export async function GET(request: NextRequest) {
  try {
    ...
    // Only get doctors created by this admin clinical
    const { data: doctors, error } = await supabase
      .from('doc_doctors')
      .select('*')
      .eq('role', 'doctor')
      .eq('created_by', authResult.doctorId)
...
async function sendCredentialsEmail({
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: email,
      subject: 'Welcome to AiSurgeonPilot - Your Login Credentials',
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            ...
            <p>Your account has been created on AiSurgeonPilot. Below are your login credentials:</p>
...
async function sendCredentialsWhatsApp({
  ...
  const message = `Welcome to AiSurgeonPilot, Dr. ${fullName}!
